...
void CMy0141121830_zhaobeichenDlg::OnBnClickedOk()
{
	...
	if (TestContent != "")
	{
		...
		check.CalSpeed(TestContent, MAXspeed, UseTime);  //计算速度
		check.Calscore(SpeedWeight, AccuracyWeight);    //计算成绩
		check.OutScore(Person);                        //输出成绩
	}
	else{
...
void CMy0141121830_zhaobeichenDlg::OnBnClickedStartTest()
{
	...
	{
		UpdateData(true);
		Person.setTestNumber(Admissino_number);           //为考生对象赋值
		...
		if (Admissino_number != "" && IDcard_number != "" && Question_number != "")
		{
			...
			else if (Person.getIDcardNumber() == "000000")         //管理人员设置密码
			{
...
void CMy0141121830_zhaobeichenDlg::OnBnClickedPrintScorePaper()
{
	// TODO:  在此添加控件通知处理程序代码
	PDFout temp;
	temp.CreatePDF(Person,TestContent,Person.getTestNumber());
	CString paths("Score\\Score_");
	CString _pdf(".pdf");
	paths = paths + Person.getTestNumber() + _pdf;
	ShellExecute(NULL, "open", paths, NULL, NULL, SW_SHOWNORMAL);
